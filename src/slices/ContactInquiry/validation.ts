...
export function validateInquiryField(name: InquiryField, value: string): string {
  const trimmed = value.trim();
  if (!trimmed) {
    return name === "gender" ? "" : "This field is required.";
  }
  const limits = { name: 120, gender: 80, phone: 40, email: 254, message: 5000 };
