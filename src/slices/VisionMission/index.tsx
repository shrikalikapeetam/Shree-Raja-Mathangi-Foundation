  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 sm:gap-7 md:gap-8 md:flex-row lg:gap-9 xl:gap-10">
        ...
        <article className="flex min-w-0 flex-col gap-4 rounded-xl border border-foundation-gold/70 bg-legacy-surface p-4 shadow-sm sm:p-5 md:rounded-2xl md:p-6 lg:p-7 xl:rounded-3xl xl:p-8">
          ...
          {isFilled.group(slice.primary.pillars) && (
            <ul className="grid list-none gap-4 p-0 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2">
              {slice.primary.pillars.map((pillar, index) =>
                isFilled.keyText(pillar.title) ||
                isFilled.keyText(pillar.description) ? (
                  <li
                    key={index}
                    className="flex min-w-0 flex-col gap-1 rounded-lg border border-foundation-gold/40 bg-foundation-white p-4 shadow-sm md:rounded-xl md:p-5 xl:rounded-2xl xl:p-6"
                  >
                    {isFilled.keyText(pillar.title) && (
